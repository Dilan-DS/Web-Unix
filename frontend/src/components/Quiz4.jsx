...
import { preguntasSemana4 } from '../data/preguntasSemana4'
...
const Quiz4 = () => {
  ...
  useEffect(() => {
    const preguntasAleatorias = obtenerPreguntasAleatorias(preguntasSemana4, 10)
  ...
  const calcularResultados = () => {
    ...
    // 🔐 Guardar nota y marcar quiz como completado
    marcarParteComoCompletada('semana4', 'quiz')
    guardarNotaQuiz('semana4', puntos)
  }

  const reiniciarQuiz = () => {
    setPreguntas(obtenerPreguntasAleatorias(preguntasSemana4, 10))
    setRespuestas({})
    setPreguntaActual(0)
    setPuntaje(0)
